...
(function () {
	...
	var DataGenerator = $.extend(function (styleFn, volumeFn, cfg) {
		...
		for (var i = 0; i < maxI; i++) {
			for (var j = 0; j < cfg.channels; j++) {
				data.push(asBytes(volumeFn(styleFn(cfg.freq, cfg.volume, i, cfg.sampleRate, cfg.seconds, maxI), cfg.freq, cfg.volume, i, cfg.sampleRate, cfg.seconds, maxI) * attack(i), 2));
			}
		}
		return data;
	}, {
	...
	function toDataURI(cfg) {
		...
		var sampleData = DataGenerator(cfg.styleFn || DataGenerator.style.default, cfg.volumeFn || DataGenerator.volume.default, cfg);
